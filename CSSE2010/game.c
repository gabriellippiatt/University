...
int8_t move_base(int8_t direction) {	
	...
	if(direction == 0 && basePosition > 0){
		redraw_base(COLOUR_BLACK);
		basePosition--;
		redraw_base(COLOUR_BASE);
		
	} else if(direction == 1 && basePosition < 7){
		redraw_base(COLOUR_BLACK);
		basePosition++;
		redraw_base(COLOUR_BASE);
	}
...
void base_hit_detection(void) {
	
	int8_t i;
	uint32_t current_base_position = basePosition, k;
	
	k=20;
	
	
	if(asteroid_at(current_base_position,1)!=-1){
		remove_asteroid(asteroid_at(current_base_position,1));
		loose_a_life(1);
		new_random_asteroids();
		for(i=0;i<3;i++){
	...
	} else if(asteroid_at(current_base_position-1,0)!= -1) {
		remove_asteroid(asteroid_at(current_base_position-1,0));
		loose_a_life(1);
		new_random_asteroids();
		for(i=0;i<3;i++){
			redraw_base(COLOUR_PROJECTILE);
...
void falling_asteroids(void) {
	...
	while(i < numAsteroids) {
		...
		if (projectile_at(x,y) == -1){
			...
			i++;
		} else if (projectile_at(x,y)!=-1) {
			redraw_asteroid(i,COLOUR_BASE);
			remove_asteroid(i);
			new_random_asteroids();
...
void accelerate_asteroids(void) {
	if (get_score() >= 0 && get_score() <= 10) {
		speed = 700;
	} else if (get_score() >= 10 && get_score() <= 20) {
		speed = 650;
	} else if (get_score() >= 20 && get_score() <= 40) {
		speed = 550;
	} else if (get_score() >= 40 && get_score() <= 60) {
		speed = 400;
	} else if (get_score() >= 60 && get_score() <= 80) {
		speed = 100;
...
void new_random_asteroids(void){
	uint8_t x;
	...
	do {
		...
		// to FIELD_HEIGHT - 1 (i.e., not in the lowest
		// three rows)
	} while(asteroid_at(x,15) != -1);
	// If we get here, we've now found an x,y location without
	// an existing asteroid - record the position
	//numAsteroids = numAsteroids + 1;
	asteroids[numAsteroids++] = GAME_POSITION(x,15);
	redraw_all_asteroids();
}
...
void advance_projectiles(void) {
	...
	while(projectileNumber < numProjectiles) {
		...
		if(y == FIELD_HEIGHT) {
			...
			// OTHERWISE...
			if (asteroid_at(x,y)!=-1 || asteroid_at(x,y-1)!=-1) {
				remove_asteroid(asteroid_at(x,y));
