...
void ILI9163_render()
{
    ...
    SPI_DMA_FL = 0;
    while (!SPI_DMA_FL) {
    } // This can be commented out if your thread sends new frames slower than SPI transmits them. Otherwise, memory havoc. See README.md
}
...
void ILI9163_drawTriangleUP(int16_t x0, int16_t y0, uint16_t color)
{
    for (int i = 0; i < 12; i++) {
        ILI9163_drawLine(x0 - i, y0 + i, x0 + i, y0 + i, color);
    }
    ILI9163_drawPixel(x0, y0, color);
}

void ILI9163_drawTriangleDOWN(int16_t x0, int16_t y0, uint16_t color)
{
    for (int i = 0; i < 12; i++) {
        ILI9163_drawLine(x0 - i, y0 - i, x0 + i, y0 - i, color);
    }
