...
public class RoutingTable {
    ...
    public void addNeighbour(Stop neighbour) {
        this.addOrUpdateEntry(neighbour, initialStop.distanceTo(neighbour),
                neighbour);
            this.synchronise();
    }
    ...
    public boolean addOrUpdateEntry(Stop destination, int newCost,
        ...
        if (!routingMap.containsKey(destination)) {
            routingMap.put(destination,
                                    new RoutingEntry(intermediate, newCost));
            return true;
        } else if (routingMap.get(destination).getCost() > newCost) {
     ...
     * @return
     * The best stop to route the passengers to in order to reach the given
     * destination.
     */
    ...
    public void synchronise() {
        List<Stop> traverseList = this.traverseNetwork();
            for (Stop mainStop : traverseList) {
                List<Stop> neighbours = mainStop.getNeighbours();
                for (Stop neighbour : neighbours) {
                    if (neighbour.getRoutingTable().transferEntries(mainStop)) {
                        break;
                    }
                }
            }
    ...
    public boolean transferEntries(Stop other) {
        ...
        for (Stop destination : routingMap.keySet()) {
            if (other.getRoutingTable().addOrUpdateEntry(destination,
                    other.distanceTo(initialStop) +
                        initialStop.distanceTo(destination), initialStop)) {
                transferStatus = true;
            }
        }
    ...
    public java.util.List<Stop> traverseNetwork() {
        ...
        // removes duplicates
        LinkedHashSet<Stop> hashSet = new LinkedHashSet<>(traversalList);
