...
void device_scancb(const bt_addr_le_t *addr, int8_t rssi, uint8_t adv_type, struct net_buf_simple *buf) {
    ...
    if(find_device(fadsArgon,addr)) {
            packet.deviceCount++;
            packet.usvals1[0] = buf->data[usBegin];
            packet.usvals1[1] = buf->data[usBegin+1];
    }

    if(find_device(gabeArgon,addr)) {
        packet.deviceCount++;
        packet.usvals2[0] = buf->data[usBegin];
        packet.usvals2[1] = buf->data[usBegin+1];
    
    }
    if(find_device(alladdys[0],addr)) {
    ...
    if(find_device(alladdys[3],addr)) {
        packet.rssiVals[3] = abs(rssi);
    }
    if(find_device(alladdys[4],addr)) {
        packet.rssiVals[4] = abs(rssi);
    }
    if(find_device(alladdys[5],addr)) {
        packet.rssiVals[5] = abs(rssi);
    }
    if(find_device(alladdys[6],addr)) {
        packet.rssiVals[6] = abs(rssi);
    }
...
void adv_thread(void) {
    ...
    while(1) {
        if (k_msgq_get(&my_msgq, &data, K_FOREVER) == 0) {
            ...
            adv_packet[12] = data.usvals1[0];
            adv_packet[13] = data.usvals1[1];
            adv_packet[14] = data.usvals2[0];
            adv_packet[15] = data.usvals2[1];
            printk("a:%f,b:%f,c:%f\n",adv_packet[0],adv_packet[1],adv_packet[2]);
        }
         if (k_msgq_get(&my_mstime, &timeRec, K_FOREVER) == 0) {
             printk("time:%d\n",timeRec.time);
             adv_packet[16] = timeRec.time >> 8;
             adv_packet[17] = timeRec.time & 0x00FF;
         }
        load(adv_packet,2,18);
        reset_devices();
        reset_beacons();
        k_sleep(K_MSEC(100));
    }
