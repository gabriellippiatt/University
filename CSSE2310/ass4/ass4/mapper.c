...
void* response_checker(void* v) {
    ...
    thread->airportCopy->idCheck = (char*)calloc(100, sizeof(char));

    while (1) {
        char fileRead[1000];
        memset(fileRead, 0, 1000);
        memset(thread->airportCopy->idCheck, 0, 
                strlen(thread->airportCopy->idCheck));
        fgets(fileRead, 1000, read);
        pthread_mutex_lock(&mutex);
        int len = strlen(fileRead);

        if (fileRead[0] == '@' && strlen(fileRead) == 2) {
            lexicographical(thread->airportCopy);
            for (int i = 0; i < thread->airportCopy->airportCounter; i++) {
                fprintf(write, "%s\n", thread->airportCopy->mappings[i]);
                fflush(write);
            }
        } else if (fileRead[0] == '?') {
            query_ports(write, read, fileRead, len, thread->airportCopy);
        } else if (fileRead[0] == '!') {
            add_mapping(len, fileRead, thread->airportCopy);
        }
