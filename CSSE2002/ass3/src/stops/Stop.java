...
public class Stop implements Writeable {
    ...
    // the routing table for this stop
    private RoutingTable routingTable;

    /**
     * Creates a new Stop object with the given name and coordinates.
     * A stop should be created with no passengers, routes, or vehicles. Its
     * routing table should initially be empty.
     *
     ...
     *
     * This neighbour should also be recorded as a neighbour
     * (RoutingTable.addNeighbour(Stop)) in the routing table for use when
     * routing passengers.
     *
     ...
     *
     * If the given passenger is null, it should not be added to the stop. If
     * the passenger's destination is null, it should be added to the stop, but
     * the stop does not need to determine where to route it.
     *
     * When a passenger arrives at a stop, the stop should use its routing table
     * to determine which neighbouring stop to route the passenger to based on
     * the passenger's desired final destination see
     * (RoutingTable.nextStop(Stop)). The stop should keep a record of where
     * each passenger waiting at it should be routed to next.
     *
    ...
    public void addPassenger(Passenger passenger) {
        ...
        this.passengers.add(passenger);
        routingTable.nextStop(passenger.getDestination());
    }
     ...
     *
     * @return The routing table for the stop.
     */
     ...
     *
     * Otherwise, this method should update the vehicle's location to be the
     * next stop (using PublicTransport.travelTo(Stop)).
     *
     * <p>This method should also update the vehicle's location to be the next
     * stop (using {@link PublicTransport#travelTo(Stop)}).
     *
     * This method should also check to see if any of the passengers currently
     * at this stop need to be routed to the next stop, and add them to the
     * transport vehicle to be taken to the next stop.
     *
     * Passengers should be added to the vehicle in the order in which they
     * arrived at the stop. If the transport vehicle reaches its capacity whilst
     * there are still passengers waiting, the remaining passengers should just
     * be left at the stop to wait for the next vehicle.
     *
    ...
    public void transportDepart(PublicTransport transport, Stop nextStop) {
        ...
        for (Passenger passenger: getWaitingPassengers()) {
            if (routingTable.nextStop(passenger.getDestination())
                                                        .equals(nextStop)) {
                for (i = 0; i < (transport.getCapacity() -
                                    transport.getPassengers().size()); i++) {
                    try {
                        transport.addPassenger(passenger);
                    } catch (OverCapacityException e) { //
                        // this is intentionally left blank as the method does
                        // not allow the passengers to be added if transport is
                        // at maximum capacity.
                    }
                }
            }
